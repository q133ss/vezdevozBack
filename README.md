...
## Установка
```bash
docker exec -it app_vezdevoz composer install
```
...
- **Запуск миграций:**
  ```bash
  docker exec -it app_vezdevoz php artisan migrate
  ```

- **Очистка кэша конфигурации:**
  ```bash
  docker exec -it app_vezdevoz php artisan config:cache
  ```

- **Создание новой миграции:**
  ```bash
  docker exec -it app_vezdevoz php artisan make:migration create_users_table
  ```

- **Заполнение базы данных данными:**
  ```bash
  docker exec -it app_vezdevoz php artisan db:seed
  ```
...
```bash
alias art="docker exec -it app_vezdevoz php artisan"
```
